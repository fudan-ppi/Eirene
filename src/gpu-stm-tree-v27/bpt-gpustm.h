#ifndef BPT_GPUSTM_H_27 
#define BPT_GPUSTM_H_27
...
namespace gpu_stm_nsp_27{
    ...
    const int SM = 80;
    const int Block_Per_SM = 512*5;
    const int Block_Dim_Put = WARPSIZE * 2;    //32*16=512
    const int Block_Dim_Get = WARPSIZE * 2;  //32*16=512
    const int Grid_Dim = Block_Per_SM * SM;     //计划中如果全是get的话的grid_dim。实际的grid_dim要后面再算


    const int tx_num = Block_Dim_Get * Grid_Dim / WARPSIZE;   //同时可以处理多少TX
                            //(默认由一个warp处理一个TX)

    const int tx_size_get = 32; // 能整除32的小于32的值 
